# Traitor Uplink Loadout Group
loadout-group-traitor-uplink = Аплинк
# PDA Uplink Option
loadout-traitor-uplink-pda-name = КПК Аплинк
loadout-traitor-uplink-pda-desc = Ваш Аплинк будет спрятан в КПК. Введите код в рингтоне для доступа к магазину.
# Pen Uplink Option
loadout-traitor-uplink-pen-name = Аплинк ручка
loadout-traitor-uplink-pen-desc = Ваш Аплинк будет спрятан в ручке. Прокрутите, чтобы ввести код и разблокировать магазин. Автоматически блокируется при закрытии.
# Implant Uplink Option
loadout-traitor-uplink-implant-name = Подкожный Аплинк
loadout-traitor-uplink-implant-desc = Ваш Аплинк будет имплантирован вам в кожу. Стоимость 10 ТК.  Активируется кнопкой на хот-баре.
ent-LoadoutDummyTraitorPda = КПК Аплинк
    .desc = Ваш Аплинк будет спрятан в КПК. Введите код в рингтоне для доступа к магазину.
ent-LoadoutDummyTraitorPen = Аплинк ручка
    .desc = Ваш Аплинк будет спрятан в ручке. Прокрутите, чтобы ввести код и разблокировать магазин. Автоматически блокируется при закрытии.
ent-LoadoutDummyTraitorImplant = Подкожный Аплинк
    .desc = Ваш Аплинк будет имплантирован вам в кожу. Стоимость 10 ТК.  Активируется кнопкой на хот-баре.
